...
/**
 * "资源束"代表了和国家, 语言及地区相关的对象.  当你的程序需要地区相关的资源, 例如字符串, 那么你的程序就可以装入一个resource bundle, 并指定你所需要的locale对象.
 * Resource bundle会按照一定的规则取得尽可能接近需要的资源.
 *
 * <p>
 * <code>ResourceBundle</code>是对<code>java.util.ResourceBundle</code>的扩展.
 * 这个<code>ResourceBundle</code>采用了和<code>java.util.ResourceBundle</code>相同的资源查找策略, 但使用了factory模式,
 * 使之更容易扩展出新的资源格式.
 * </p>
 *
 * <p>
 ...
 * 则baseName_language1_country1_variant1以及baseName_language1_country1被忽略. 如果全部locale元素均为空,
 * 则只搜索baseName.
 * </p>
 *
...
public abstract class ResourceBundle extends java.util.ResourceBundle {
    ...
    /**
     * 从当前resource bundle或它的一个父bundle中, 取得和指定键对应的字符串.  如果不成功,
     * 则掷出<code>MissingResourceException</code>异常.
     *
     * @param key  要查找的键
     *
     * @return key对应的字符串
    ...
    /**
     * 从当前resource bundle或它的一个父bundle中, 取得和指定键对应的<code>Map</code>.  如果不成功,
     * 则掷出<code>MissingResourceException</code>异常.
     *
     * @param key  要查找的键
     *
    ...
    /**
     * 从当前resource bundle或它的一个父bundle中, 取得和指定键对应的<code>List</code>.  如果不成功,
     * 则掷出<code>MissingResourceException</code>异常.
     *
     * @param key  要查找的键
     *
    ...
    /**
     * 从当前resource bundle或它的一个父bundle中, 取得和指定键对应的对象.  如果不成功,
     * 则掷出<code>MissingResourceException</code>异常.
     *
     * @param key  要查找的键
     *
